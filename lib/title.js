...
const { assertPublicUrl } = require('./utils');
...
async function streamHtmlHead(url, signal) {
...
async function tryMicrolink(url, signal) {
  try {
    const res = await fetchWithSafeRedirects(`https://api.microlink.io?url=${encodeURIComponent(url)}`, {
      headers: { 'User-Agent': BROWSER_UA },
      signal,
    });
...
async function fetchTitle(url, host) {
  ...
  const oembedTitle = await tryOembed(url, host, signal);
  ...
  const html = await streamHtmlHead(url, signal).catch(() => '');
  if (html) {
  ...
  const microlinkTitle = await tryMicrolink(url, signal);
