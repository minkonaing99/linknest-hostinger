...
const { makeId, makeSessionToken, isApiPath, publicUser } = require('./utils');
...
async function createApiToken(user, name, scope = 'write', expiresAt = null) {
  ...
  await query(
    `INSERT INTO api_tokens (id, user_id, name, token_hash, scope, created_at, last_used_at, expires_at, revoked_at)
     VALUES (?, ?, ?, ?, ?, ?, NULL, ?, NULL)`,
    [makeId(), user.id, String(name).trim(), hashToken(rawToken), normalizedScope, now, expiresAt ? new Date(expiresAt) : null]
  );
  return rawToken;
}
