...
function normalizeUrl(input) {
  ...
  // Canonicalize protocol: http -> https
  if (url.protocol === 'http:') url.protocol = 'https:';

  // Strip mobile/AMP subdomains: m., mobile., amp.
  url.hostname = url.hostname.replace(/^(m|mobile|amp)\./i, '');

  const noisyParams = new Set([
    'fbclid', 'gclid', 'igshid', 'mc_eid', 'mc_cid', 'mkt_tok',
    'ref', 'ref_src', 'ref_url', 'si',
    '_ga', 'yclid', 'dclid', 'msclkid', 'twclid', 'ttclid', 'sccid',
  ...
  for (const key of [...url.searchParams.keys()]) {
    if (noisyParams.has(key.toLowerCase()) || /^utm_/i.test(key)) {
...
function youtubeThumbnailUrl(urlString) {
  let url;
  try { url = new URL(urlString); } catch { return null; }

  const host = url.hostname.toLowerCase();
  const mainHosts = new Set(['youtube.com', 'www.youtube.com', 'm.youtube.com']);
  ...
  } else if (mainHosts.has(host)) {
    if (url.pathname === '/watch') videoId = url.searchParams.get('v');
    else {
      const match = url.pathname.match(/^\/(?:shorts|embed|live)\/([^/]+)\/?$/);
      videoId = match?.[1] || null;
    }
  } else if (embedHosts.has(host)) {
...
function buildSort(sort, order) {
  ...
  if (sort === 'createdAt') return `created_at ${dir}, updated_at DESC, id ASC`;
...
function parseLinkListQuery(searchParams) {
  ...
  const remindBefore  = parseIsoDateTime(searchParams.get('remindBefore'),  'remindBefore');
  const staleBefore   = parseIsoDateTime(searchParams.get('staleBefore'),   'staleBefore');
  const ageBefore     = parseIsoDateTime(searchParams.get('ageBefore'),     'ageBefore');
  const neverOpened   = parseBooleanFlag(searchParams.get('neverOpened'), false);
  const youtube       = String(searchParams.get('youtube') || '').trim();
  if (youtube && !['only', 'exclude'].includes(youtube)) {
    throw validationError('youtube must be only or exclude');
  }
  ...
  if (youtube) {
    params.push('youtube.com', 'm.youtube.com', 'youtu.be', 'youtube-nocookie.com');
    conditions.push(`host ${youtube === 'only' ? '' : 'NOT '}IN (?, ?, ?, ?)`);
  }
  ...
  if (remindBefore) {
    params.push(remindBefore);
    conditions.push('remind_at IS NOT NULL AND remind_at <= ?');
  }

  if (staleBefore) {
    params.push(staleBefore, staleBefore);
    conditions.push('(last_opened_at < ? OR (last_opened_at IS NULL AND created_at < ?))');
  }

  if (ageBefore) {
    params.push(ageBefore);
    conditions.push("status IN ('saved', 'unread')");
    conditions.push('created_at <= ?');
    conditions.push('(first_meaningful_at IS NULL OR first_meaningful_at < DATE_ADD(created_at, INTERVAL 1 DAY))');
    conditions.push('(remind_at IS NULL OR remind_at <= NOW())');
  }

  if (neverOpened) {
    conditions.push('opened_count = 0');
  }

  if (q) {
    const like = `%${q}%`;
    params.push(like, like, like, like, like, like);
    conditions.push('(title LIKE ? OR url LIKE ? OR host LIKE ? OR notes LIKE ? OR JSON_SEARCH(tags, \'one\', ?) IS NOT NULL OR date LIKE ?)');
  }
...
function sanitizeEntry(input = {}, options = {}) {
  ...
  const date = String(body.date || thailandDateString()).trim();
  ...
  const remindAt  = body.remindAt  == null || body.remindAt  === '' ? null : parseIsoDateTime(body.remindAt,  'remindAt');
  if (body.notes != null && typeof body.notes !== 'string') {
    throw validationError('notes must be plain text');
  }
  const notes = String(body.notes || '').trim();
  if (notes.length > ENTRY_NOTES_MAX_LENGTH) {
...
function normalizeStoredEntry(item = {}) {
  return {
    ...
    firstMeaningfulAt: item.firstMeaningfulAt || null,
    firstUsefulAt: item.firstUsefulAt || null,
    lastUsefulReviewedAt: item.lastUsefulReviewedAt || null,
    thumbnailUrl: youtubeThumbnailUrl(item.url || ''),
  };
}
...
module.exports = {
  ...
  normalizeUrl, deriveHost, youtubeThumbnailUrl, thailandDateString, jaroWinkler,
