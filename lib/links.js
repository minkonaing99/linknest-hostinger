  ...
  await query(
    `UPDATE links
     SET url=?, title=?, host=?, status=?, tags=?, pinned=?,
         ...
         first_meaningful_at=CASE WHEN ? THEN COALESCE(first_meaningful_at, ?) ELSE first_meaningful_at END
     WHERE id=?`,
    [entry.url, entry.title, entry.host, entry.status, JSON.stringify(entry.tags),
     entry.pinned ? 1 : 0, entry.date, entry.createdAt, entry.updatedAt, entry.deletedAt, entry.remindAt, entry.notes,
     becameMeaningful ? 1 : 0, entry.updatedAt, id]
  );
  return normalizeStoredEntry({
    ...entry,
    firstMeaningfulAt: current.firstMeaningfulAt || (becameMeaningful ? entry.updatedAt : null),
  });
}
  ...
  if (options.hardDelete) {
    ...
    await query(
      `UPDATE links SET deleted_at=?, updated_at=?, status='archived', pinned=0,
       first_meaningful_at=COALESCE(first_meaningful_at, ?) WHERE id=?`,
...
async function bulkUpdateStatus(ids, status) {
  ...
  if (normalized === 'useful') params.push(updatedAt);
  params.push(ids);
  const result = await query(
