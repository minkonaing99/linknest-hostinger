...
async function updateLink(id, body) {
  ...
  const qualifiesAsRevisit = becameMeaningful
    && Date.parse(entry.updatedAt) - Date.parse(current.createdAt) >= 24 * 60 * 60 * 1000;
...
async function bulkUpdateStatus(ids, status) {
  ...
  if (normalized === 'useful') params.push(dbDate(updatedAt), dbDate(updatedAt));
  params.push(ids);
  const result = await query(
