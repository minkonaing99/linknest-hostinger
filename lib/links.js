...
async function createLink(input) {
  ...
  await query(
    `INSERT INTO links (id, url, title, host, status, tags, pinned, date, created_at, updated_at, deleted_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
async function updateLink(id, body) {
  ...
  await query(
    ...
    [entry.url, entry.title, entry.host, entry.status, JSON.stringify(entry.tags),
     entry.pinned ? 1 : 0, entry.date, entry.createdAt, entry.updatedAt, entry.deletedAt, id]
  );
  return normalizeStoredEntry(entry);
}
