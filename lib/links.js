...
async function createLink(input) {
  ...
  await query(
    ...
    [entry.id, entry.url, entry.title, entry.host, entry.status, JSON.stringify(entry.tags),
     entry.pinned ? 1 : 0, entry.date, entry.createdAt, entry.updatedAt, entry.deletedAt, entry.remindAt, entry.notes]
  );
    ...
    try {
      ...
      const result = await query(
        `INSERT IGNORE INTO links (id, url, title, host, status, tags, pinned, date, created_at, updated_at, deleted_at, notes)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [entry.id, entry.url, entry.title, entry.host, entry.status, JSON.stringify(entry.tags),
         entry.pinned ? 1 : 0, entry.date, entry.createdAt, entry.updatedAt, entry.deletedAt, entry.notes]
      );
...
async function updateLink(id, body) {
  ...
  await query(
    ...
    [entry.url, entry.title, entry.host, entry.status, JSON.stringify(entry.tags),
     entry.pinned ? 1 : 0, entry.date, entry.createdAt, entry.updatedAt, entry.deletedAt, entry.remindAt, entry.notes,
     qualifiesAsRevisit ? 1 : 0, entry.updatedAt, id]
  );
    ...
    await query(
      `UPDATE links SET deleted_at=?, updated_at=?, status='archived', pinned=0,
       first_meaningful_at=CASE
         ...
         THEN ? ELSE first_meaningful_at END
       WHERE id=?`,
      [deletedAt, deletedAt, deletedAt, deletedAt, id]
    );
  }
...
async function bulkUpdateStatus(ids, status) {
  ...
  const meaningfulSql = normalized === 'useful'
    ? `, first_meaningful_at=CASE
        ...
        THEN ? ELSE first_meaningful_at END`
    : '';
  const params = [normalized, updatedAt];
  if (normalized === 'useful') params.push(updatedAt, updatedAt);
  params.push(ids);
  const result = await query(
