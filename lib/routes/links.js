...
const {
  ...
  readReviewQueue, readUsefulReviewQueue, markUsefulReviewed, mergeLinkNote,
...
function mergeNoteId(pathname) {
  for (const prefix of ['/api/links/', '/api/v1/links/']) {
    if (!pathname.startsWith(prefix)) continue;
    const parts = pathname.slice(prefix.length).split('/');
    if (parts.length === 2 && parts[1] === 'merge-note') return decodeURIComponent(parts[0]);
  }
  return null;
}

function usefulReviewId(pathname) {
  for (const prefix of ['/api/links/', '/api/v1/links/']) {
    if (!pathname.startsWith(prefix)) continue;
    const parts = pathname.slice(prefix.length).split('/');
    if (parts.length === 2 && parts[1] === 'useful-review') return decodeURIComponent(parts[0]);
  }
  return null;
}

function relatedPath(pathname) {
  for (const prefix of ['/api/links/', '/api/v1/links/']) {
    if (!pathname.startsWith(prefix)) continue;
    const parts = pathname.slice(prefix.length).split('/');
    if (parts.length < 2 || parts[1] !== 'related') continue;
    if (parts.length === 2) return { id: decodeURIComponent(parts[0]), relatedId: null };
    if (parts.length === 3) {
      return { id: decodeURIComponent(parts[0]), relatedId: decodeURIComponent(parts[2]) };
    }
  }
  return null;
}
...
async function handleReview(req, res) {
  try {
    const links = await readReviewQueue();
...
async function handleUsefulReview(req, res) {
  try {
    const links = await readUsefulReviewQueue();
    sendJson(res, 200, { links });
  } catch (error) {
    sendJson(res, error.statusCode || 500, error.payload || { error: error.message });
  }
...
async function handleUsefulReviewed(req, res, id) {
  try {
    const body = ensurePlainObject(await parseBody(req));
    const entry = await markUsefulReviewed(id, body);
...
async function handleMergeNote(req, res, id) {
  try {
    const body = ensurePlainObject(await parseBody(req));
    const entry = await mergeLinkNote(id, body.note);
    sendJson(res, 200, { ok: true, entry });
  } catch (error) {
    sendJson(res, error.statusCode || 400, error.payload || { error: error.message });
  }
...
async function handleRelated(req, res, method, path) {
  try {
    if (method === 'GET' && path.relatedId === null) {
      sendJson(res, 200, { links: await readRelatedLinks(path.id) });
      return;
    }
    if (method === 'POST' && path.relatedId === null) {
      const body = ensurePlainObject(await parseBody(req));
      if (typeof body.relatedId !== 'string' || !body.relatedId.trim()) {
        sendJson(res, 400, { error: 'relatedId is required' });
        return;
      }
      sendJson(res, 201, { link: await addRelatedLink(path.id, body.relatedId.trim()) });
      return;
    }
    if (method === 'DELETE' && path.relatedId) {
      sendJson(res, 200, await removeRelatedLink(path.id, path.relatedId));
    }
  } catch (error) {
    sendJson(res, error.statusCode || 400, error.payload || { error: error.message });
  }
...
async function handle(req, res, reqUrl) {
  ...
  const mid = mergeNoteId(p);
  const uid = usefulReviewId(p);
  const related = relatedPath(p);

  if (m === 'GET'   && isBase(p))                                                                          return handleList(req, res, reqUrl), true;
  if (m === 'POST'  && isBase(p))                                                                          return handleCreate(req, res), true;
  if (related && (((m === 'GET' || m === 'POST') && related.relatedId === null)
    || (m === 'DELETE' && related.relatedId))) return handleRelated(req, res, m, related), true;
  ...
  if (m === 'POST'  && mid)                                                                                return handleMergeNote(req, res, mid), true;
  if (m === 'POST'  && uid)                                                                                return handleUsefulReviewed(req, res, uid), true;
  if (m === 'PATCH' && is(p, '/api/links/bulk', '/api/v1/links/bulk'))                                    return handleBulk(req, res), true;
  if (m === 'GET'   && is(p, '/api/links/review', '/api/v1/links/review'))                                return handleReview(req, res), true;
  if (m === 'GET'   && is(p, '/api/links/useful-review', '/api/v1/links/useful-review'))                  return handleUsefulReview(req, res), true;
